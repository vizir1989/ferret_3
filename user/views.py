...
import jwt
from flask import jsonify, Blueprint, make_response, current_app
from flask import request, session
from flask_login import logout_user, current_user, login_user

from ferret import db, lm
from user.models import User
from user.serialization import UserInputSchema

bp = Blueprint("user", __name__, url_prefix="/user")
create_user_schema = UserInputSchema()


@bp.route('/signup', methods=['POST'])
def signup():
    """User input token route."""
    try:
        errors = create_user_schema.validate(request.form)
        if errors:
            return jsonify(message=str(errors), result=False)
